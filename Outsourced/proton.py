...
# enter the difference of the value t0-t1, t1-t2, t2-t3...

# hyperopt must be performed ONLY on periods of lateral movement and TESTED on medium bull and medium bear periods
# (EXPLANATION: if I hyperopt over a long period of time, it may try to optimize certain extreme conditions such as pump and dump to maximize profits on individual trades: 
# the question is that we could focus on specific and rare events that compensate for everything else)

# insert these indicators: https://github.com/just-nilux/legendary_ta/tree/main

# I would modify the target to try to reach the trigger of the positive trailing and I would decrease the shift -50 which are too many candles in the future!!! -> let's see if it gives more importance to smaller timeframes instead of 15m by decreasing -50
# modify the target to handle longs and shorts
